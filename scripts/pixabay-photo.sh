#!/usr/bin/env bash
# pixabay-photo.sh — fetch a stock photo from Pixabay's JSON API.
#
#   scripts/pixabay-photo.sh "<query>" <out.jpg> [orientation=horizontal|vertical|all] [min_w=1920]
#
# License: Pixabay Content License (commercial-OK, no attribution).
# Writes <out>.license.json next to the image.
set -euo pipefail
Q="${1:?usage: pixabay-photo.sh <query> <out.jpg> [orient=horizontal|vertical|all] [min_w=1920]}"
...
URL="https://pixabay.com/api/?key=${KEY}&q=${QENC}&image_type=photo&orientation=${ORIENT}&min_width=${MINW}&safesearch=true&per_page=30"
...
best = max(hits, key=lambda h: (h.get('imageWidth',0) * h.get('imageHeight',0), h.get('downloads',0)))
print(json.dumps({
...
PY
) || { echo "[pixabay-photo] no match for '$Q'" >&2; exit 3; }
