...
command -v ffmpeg >/dev/null || { echo "!! ffmpeg required (brew install ffmpeg / apt install ffmpeg / winget install Gyan.FFmpeg)"; exit 1; }
...
$PY -m pip install $PYFLAGS openai-whisper soundfile 2>/dev/null \
  || $PY -m pip install --user openai-whisper soundfile \
