---
name: b-roll
description: Places b-roll cutaways and enforces clean boundaries — graphics finish BEFORE b-roll starts, b-roll plays naked, post-b-roll graphics start AFTER b-roll ends. Use whenever a video has cutaways. Runs after stock-scout, before motion-builder + assembler finalize timing.
...
- Give the assembler the filter_complex snippet: per cutaway, `[clip]scale=W:H:force_original_aspect_ratio=increase,crop=W:H,fps=<f>,trim=0:DUR,setpts=PTS-STARTPTS+IN/TB,setsar=1[c]; [prev][c]overlay=eof_action=pass:enable='between(t,IN,OUT)'[next]`.
...
**Hand off to:** motion-builder (suppress scene overlays inside windows) → assembler (composite via filter_complex) → qa-visual.
