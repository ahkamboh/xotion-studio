...
## Sticker Pop — cobalt + pink bubble + yellow mascot, Y2K kawaii sheet
Use: playful brand reels, social-first cute promos, Gen-Z brand voice, sticker-pack aesthetic, anything that wants to feel like a hand-drawn Instagram fridge magnet. Fonts: **Bowlby One** or **Bagel Fat One** (chunky bubble display, 240–420px) — fallback to **Archivo Black**; **JetBrains Mono** (small kicker, 28px +0.18em UPPERCASE); **Inter** body (rare, 22–26px). Palette: kawaii sticker — cobalt blue `#2069E2` (bg), bubblegum pink `#F082C5` (hero fill), sunny yellow `#F5E531` (mascot fill), leaf green `#3DBE5B` (accent), cream highlight `#FFF6C0`, ink outline `#0F0825`. Layout: ONE big sticker per scene centered, mascot illustration behind/beside (cartoon lemon · cloud · star · smiley blob), scattered white 4-point sparkle stars (`✦` characters or SVG diamonds), thick black outlines (8px), **double-offset shadow on type** (cream `+6px` highlight, ink `+18px` shadow — no blur). Texture: slight grain (.08), NO halftone (clean flat sticker), optional sticker-sheet die-cut white outline around hero. Motion: BOUNCY — `back.out(3)` slam entrances with squash-stretch (scaleY 0.6 → 1.15 → 1), wiggle idle (`Rich.idle` rot:2,scale:.03), sparkle stars BLINK on/off (opacity 0↔1 every .35s, staggered), mascot continuous slow wobble. **MUST use `templates/lib/richness.js`** (idle on every hero). Ratio: 9:16 primary, 1:1 secondary. Sources: Wendy's social design system, Liquid Death brand, MSCHF / kawaii sticker-pack aesthetic, Bagel Fat One bubble-type revival 2024.
