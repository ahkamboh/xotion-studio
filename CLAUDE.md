...
- `templates/liquid-glass-feature.html` — Apple-style **Liquid Glass** product/feature showcase: a frosted-clear glass panel with real edge **lensing** (SVG feDisplacementMap refracts the wallpaper, not just blur) + specular sweep over an animated mesh gradient; a glowing ring + glass callout highlights one feature chip. Variables: `appName`, `headline`, `chips` (pipe-sep), `highlight` (index), `calloutText`, `accent`, gradient `g1/g2/g3`. For AI-site / product-demo UI feature highlights.
